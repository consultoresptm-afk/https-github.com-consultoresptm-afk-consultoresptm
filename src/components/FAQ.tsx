...
const faqData: FAQCategory[] = [
  {
    title: "Certificaciones ISO y Normatividad",
    items: [
      {
        id: "iso-9001",
      ...
      {
        id: "iso-9001-cost",
        question: "¿Cuánto cuesta certificar una PYME en ISO 9001 en Colombia en 2024?",
        ...
        <div className="space-y-16">
          {faqData.map((category, catIdx) => (
            <div key={catIdx} className="space-y-8">
              <h2 className="text-xl sm:text-2xl font-display text-teal border-b border-primary-foreground/10 pb-2 uppercase tracking-wide">
                {category.title}
              </h2>
                  ...
                  return (
                    <div
                      ...
                      className={`border rounded-2xl overflow-hidden transition-all duration-300 ${
                        isOpen
                          ? "bg-white/[0.04] border-teal/30 shadow-lg shadow-teal/5"
                          : "bg-white/[0.01] border-white/5 hover:bg-white/[0.02] hover:border-white/10"
                      }`}
                    >
                      <button
                        onClick={() => toggleAccordion(item.id)}
                        className="w-full text-left py-4 sm:py-5 px-5 sm:px-6 flex items-center justify-between gap-4 font-display text-base sm:text-lg text-primary-foreground font-medium transition-colors hover:text-teal focus:outline-none"
                      >
                        <span className="leading-snug">{item.question}</span>
                        <ChevronRight
                          className={`w-5 h-5 text-teal shrink-0 transition-transform duration-300 ${
                            isOpen ? "rotate-90 text-teal" : "text-primary-foreground/40"
                          }`}
                        />
                      </button>
